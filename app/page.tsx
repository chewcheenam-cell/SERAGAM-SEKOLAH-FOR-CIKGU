  ...
  return (
    ...
    <div className="space-y-3 md:hidden">
      {rows.length ? rows.map((row) => {
        const totalWithPostage = row.totalPrice + deliveryPerPerson;
        return (
          <article key={row.id} className="rounded-lg border border-batikara-line bg-white p-4">
            <div className="mb-3 flex items-start justify-between gap-3">
              <input value={row.nama} onChange={(event) => onUpdateRow(row.id, { nama: event.target.value })} className="min-w-0 flex-1 rounded-md border border-batikara-line px-2 py-2 font-bold outline-none focus:border-batikara-blue" />
              <button onClick={() => onRemoveRow(row.id)} className="rounded-md border border-red-200 px-3 py-2 text-sm font-semibold text-red-700">Remove</button>
            </div>
            <div className="grid gap-3">
              <input value={row.jawatan} onChange={(event) => onUpdateRow(row.id, { jawatan: event.target.value })} placeholder="Jawatan" className="rounded-md border border-batikara-line px-2 py-2 outline-none focus:border-batikara-blue" />
              <div className="grid grid-cols-2 gap-2">
                <select value={row.jenisPakaian} onChange={(event) => onUpdateRow(row.id, { jenisPakaian: event.target.value })} className="rounded-md border border-batikara-line px-2 py-2 outline-none focus:border-batikara-blue">
                  {ITEM_OPTIONS.map((item) => <option key={item} value={item}>{item}</option>)}
                </select>
                <select value={row.saiz} onChange={(event) => onUpdateRow(row.id, { saiz: event.target.value })} className="rounded-md border border-batikara-line px-2 py-2 outline-none focus:border-batikara-blue">
                  <option value="">Size</option>
                  {SIZE_OPTIONS.map((size) => <option key={size} value={size}>{getSizeLabel(size)}</option>)}
                </select>
              </div>
              <div className="grid grid-cols-2 gap-2">
                <label className={`inline-flex items-center justify-center gap-2 rounded-md border px-3 py-2 font-semibold ${row.poket ? "border-blue-200 bg-blue-50 text-batikara-navy" : "border-batikara-line bg-white text-slate-600"}`}>
                  <input type="checkbox" checked={row.poket} onChange={(event) => onUpdateRow(row.id, { poket: event.target.checked })} className="h-4 w-4 accent-batikara-blue" />
                  Poket +RM3
                </label>
                <label className={`inline-flex items-center justify-center gap-2 rounded-md border px-3 py-2 font-semibold ${row.paid ? "border-green-200 bg-green-50 text-green-700" : "border-amber-200 bg-amber-50 text-amber-700"}`}>
                  <input type="checkbox" checked={row.paid} onChange={(event) => onUpdateRow(row.id, { paid: event.target.checked })} className="h-4 w-4 accent-batikara-blue" />
                  {row.paid ? "Paid" : "Not Yet"}
                </label>
              </div>
              <div className="grid grid-cols-2 gap-2">
                <label className="text-sm font-semibold text-slate-600">
                  Qty
                  <input type="number" min="1" value={row.quantity} onChange={(event) => onUpdateRow(row.id, { quantity: Math.max(1, Number(event.target.value)) })} className="mt-1 w-full rounded-md border border-batikara-line px-2 py-2 outline-none focus:border-batikara-blue" />
                </label>
                <label className="text-sm font-semibold text-slate-600">
                  Unit
                  <input type="number" min="0" step="0.01" value={row.unitPrice} onChange={(event) => onUpdateRow(row.id, { unitPrice: Number(event.target.value) })} className="mt-1 w-full rounded-md border border-batikara-line px-2 py-2 outline-none focus:border-batikara-blue" />
                </label>
              </div>
              <div className="grid grid-cols-3 gap-2 rounded-md bg-batikara-sky p-3 text-sm">
                <InfoMini label="Item" value={formatCurrency(row.totalPrice)} />
                <InfoMini label="Postage" value={formatCurrency(deliveryPerPerson)} />
                <InfoMini label="Total" value={formatCurrency(totalWithPostage)} />
              </div>
            </div>
          </article>
        );
      }) : <p className="rounded-lg border border-batikara-line bg-white px-3 py-8 text-center text-slate-500">Upload a cikgu list or add names manually.</p>}
      <div className="rounded-lg border border-batikara-line bg-batikara-sky p-4">
        <label className="block text-sm font-bold text-batikara-navy">
          Total Postage Fee
          <input type="number" min="0" step="0.01" value={deliveryTotal} onChange={(event) => onDeliveryChange(Math.max(0, Number(event.target.value) || 0))} className="mt-2 w-full rounded-md border border-batikara-line px-2 py-2 outline-none focus:border-batikara-blue" />
        </label>
        <p className="mt-2 text-sm font-semibold text-batikara-navy">{formatCurrency(deliveryTotal)} divided by {summary.totalPax || 0} orang = {formatCurrency(deliveryPerPerson)} each</p>
        <p className="mt-2 text-lg font-bold text-batikara-navy">Final Total {formatCurrency(summary.grandTotal)}</p>
      </div>
    </div>
    </>
  );
...
function InfoMini({ label, value }: { label: string; value: string }) {
  return (
    <div>
      ...
      <p className="font-bold text-batikara-navy">{value}</p>
    </div>
  );
}
