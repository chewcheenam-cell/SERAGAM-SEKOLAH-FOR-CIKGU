    ...
    <main className="min-h-screen bg-[#f6f9fd] px-4 py-6 text-batikara-ink">
      <section className="print-surface mx-auto max-w-6xl rounded-lg border border-batikara-line bg-white p-5 shadow-panel">
        ...
        <div className="space-y-3 md:hidden">
            ...
            return (
              <article key={row.id} className="rounded-lg border border-batikara-line bg-white p-4">
                <div className="mb-3 flex items-start justify-between gap-3">
                  <input value={row.nama} onChange={(event) => updateRow(row.id, { nama: event.target.value })} className="min-w-0 flex-1 rounded-md border border-batikara-line px-2 py-2 font-bold outline-none focus:border-batikara-blue" />
                  <label className={`inline-flex items-center gap-2 rounded-md border px-3 py-2 text-sm font-semibold ${row.paid ? "border-green-200 bg-green-50 text-green-700" : "border-amber-200 bg-amber-50 text-amber-700"}`}>
                    <input type="checkbox" checked={row.paid} onChange={(event) => updateRow(row.id, { paid: event.target.checked })} className="h-4 w-4 accent-batikara-blue" />
                    {row.paid ? "Paid" : "Not Yet"}
                  </label>
                </div>
                <div className="grid gap-3">
                  <div className="grid grid-cols-2 gap-2">
                    <select value={row.jenisPakaian} onChange={(event) => updateRow(row.id, { jenisPakaian: event.target.value })} className="rounded-md border border-batikara-line px-2 py-2 outline-none focus:border-batikara-blue">
                      {ITEM_OPTIONS.map((item) => <option key={item} value={item}>{item}</option>)}
                    </select>
                    <select value={row.saiz} onChange={(event) => updateRow(row.id, { saiz: event.target.value })} className="rounded-md border border-batikara-line px-2 py-2 outline-none focus:border-batikara-blue">
                  ...
                  <div className="grid grid-cols-3 gap-2 rounded-md bg-batikara-sky p-3 text-sm">
                    ...
                    <InfoMini label="Postage" value={formatCurrency(summary.deliveryPerPerson)} />
                    <InfoMini label="Total" value={formatCurrency(totalPay)} />
                  </div>
                  <div>
                    <label className="inline-flex cursor-pointer items-center gap-2 rounded-md border border-batikara-line px-3 py-2 font-semibold text-batikara-navy">
                      <Upload className="h-4 w-4" />
                    ...
                    {row.slipName ? (
                      <div className="mt-2 flex items-center gap-3 text-xs text-slate-600">
                        {isImageSlip(row.slipDataUrl) ? <img src={row.slipDataUrl} alt={row.slipName} className="h-14 w-20 rounded-md border border-batikara-line object-cover" /> : null}
                        <button type="button" onClick={() => setPreviewSlip(row)} className="inline-flex items-center gap-1 rounded-md border border-batikara-line px-2 py-1 font-semibold text-batikara-blue">
                          <Eye className="h-3.5 w-3.5" />
                          View
                        </button>
                        <span className="min-w-0 flex-1 truncate">{row.slipName}</span>
                      </div>
                    ) : null}
                  </div>
                </div>
          ...
          {summary.deliveryTotal ? (
            <div className="rounded-lg border border-batikara-line bg-batikara-sky p-4 text-sm font-bold text-batikara-navy">
              Delivery: {formatCurrency(summary.deliveryTotal)} divided by {summary.total} orang = {formatCurrency(summary.deliveryPerPerson)} each
              <div className="mt-1 text-lg">Final Total {formatCurrency(summary.grandTotal)}</div>
            </div>
          ) : null}
        </div>
